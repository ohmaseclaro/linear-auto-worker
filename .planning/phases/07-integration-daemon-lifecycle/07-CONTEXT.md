...
| P5 | **`onProgress` is unwired.** `event-router.ts` routes `task_summary`/`post_turn_summary` into progress callbacks, but nothing threads `onProgress` through `supervisor.ts` / `RunAgentInput`. 04-04 does not own that file and 04-05 did not add it. | Progress milestones never reach Linear (NOTF-03) — the ticket goes quiet between pickup and terminal, which reads as a hung run. | Thread it at the composition root. Prefer `router.denials` over `resultEvent.permission_denials` for diagnosis composition. |
| P6 | **`timedOut` is unthreaded** (T61) — `runAgent` returns it, `classifyOutcome` accepts it, `execute-run.ts` does not pass it. | A run killed at its timeout with commits present classifies `delivered`, shipping a truncated PR described as complete. | One line at the `execute-run.ts` call site. |
| P7 | **`findOpenPrUrl` should use `gh pr list --json number,url`** (T60). The current text parse exists only because a grep gate banned `--json` file-wide; `gh pr create` has no `--json`, but `gh pr list` does (verified, gh 2.98.0). | An untestable parse of non-TTY output on the delivery path. | Rewrite the call and narrow the gate to `gh pr create`. |
| P8 | **CONF-02's question-flow toggle is global-only in practice.** `MappingToggles.questionsEnabled` exists in `src/domain/types.ts:185`, but the wizard's mapping step (08-03) never prompts for it — it rendered "question flow" as `questionTimeoutMs` only. | The contract satisfies CONF-02; the wizard does not. A per-mapping question-flow override cannot actually be set by the operator. | Add the prompt, or record deliberately that the toggle is config-file-only. Do not leave it ambiguous. |
| P9 | **`registrar.reconcile()` returns `{id, secret, url}`; the `WebhookRegistrar` port declares `{webhookId, secret}`.** | Adapter mismatch at the composition root. | Rename at the seam. |
| P10 | **`runSetupWizard()` requires `{tunnel, registrar, port}`** and returns exit 1 with a fix string if they are absent — deliberately, so it never prints "setup complete" over a system that would receive nothing. | Not a bug; a wiring requirement. | Supply all three at the composition root. |
| P11 | **Three production double-casts are drift, not boundaries.** Of 11 `as unknown as` in production code, 9 sit at genuine trust boundaries (SQLite rows, webhook wire payload, stream events) and are the honest answer. Three are not: `mapping.ts` and `config-writer.ts` cast to bridge cross-phase shape drift, and **`config.ts` casts twice *after* zod validation** — if a validated schema's output still needs a cast, the schema and the type have diverged. | The first two are the drift Phase 7 exists to remove. The third means config validation is weaker than it appears: zod is checking a shape the code then overrides. | Reconcile the two bridges properly; make the zod schema's inferred output match `Config` so the casts in `config.ts` disappear. Baseline in `07-TYPE-ESCAPE-BASELINE.txt`. |

**Guard on the 07-02 fix:** driving 68 errors to zero must not raise the escape-hatch counts in
`07-TYPE-ESCAPE-BASELINE.txt` (`: any` 7, `as any` 10 — only 1 in production — `as unknown as` 56
with 45 in tests, `@ts-ignore` 0, `@ts-expect-error` 0). A reconciliation that silences rather than
resolves shows up there, never in the error count. Zero suppression comments today; keep it zero.
